...
deepfake_model = Wav2Vec2ForSequenceClassification.from_pretrained(model_name, token=HF_TOKEN)
        ...
        while self.is_recording:
            try:
                ...
                if buffer_duration >= self.AUDIO_SEGMENT_DURATION:
                    ...
                    with torch.no_grad():
                        logits = deepfake_model(waveform).logits
                        probabilities = torch.nn.functional.softmax(logits, dim=-1)
                    
                    label_map = {0: "Fake", 1: "Real"}
                    predicted_class = torch.argmax(probabilities, dim=-1).item()
                    confidence = probabilities[0, predicted_class].item()
                    prediction = f"{label_map[predicted_class]} (Confidence: {confidence:.2f})"
                    
                    self.socketio.emit('deepfake_analysis', {'deepfake_analysis': prediction})
                    print(f"🔍 Deepfake Analysis: {prediction}")
